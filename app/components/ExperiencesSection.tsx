...
import { experiences, experienceCategories, sortExperiences, formatDate } from "../data/experiences";
import { AnimatePresence, motion } from "framer-motion";

export default function ExperiencesSection() {
  const groups = experienceCategories.map((category) => ({
    ...category,
    items: sortExperiences(experiences.filter((e) => e.category === category.id)),
  }));
    ...
    <section id="experiences" className="relative w-full bg-gradient-to-b from-[var(--bg-alt)] to-[var(--background)] pt-16 md:pt-20 pb-24 md:pb-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {groups.map((group, groupIndex) => (
          <div key={group.id} className={groupIndex === 0 ? "mt-8" : "mt-12"}>
            <h3 className="mb-3 px-4 text-sm font-semibold uppercase tracking-[0.12em] text-gray-500 md:pl-20 md:pr-8">
              {group.label}
            </h3>
